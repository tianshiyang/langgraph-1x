...
def demo_values():
    ...
    ══════════════════════════════════════════════════════════════
    ⭐️⭐️⭐️ updates vs values 到底怎么选（实战决策指南）

    一句话先记牢：
       updates = 「这个节点【改了什么】」——差量、轻量、关注【变化】
       values  = 「这一步之后【整体长啥样】」——全量、完整、关注【全貌】

    ── 它俩共同点 ──
       触发时机一样：都是【每个节点跑完】吐一次，不是节点中途（中途用 custom）。
       区别只在「吐差量」还是「吐全量」。

    ┌─ 选 updates 的场景（90% 的日常都用它）─────────────────────┐
    │ ① 做步骤级进度提示：「正在分类… 正在检索… 正在生成…」          │
    │    —— 你只关心「现在哪个节点动了」，不需要整个 state。          │
    │ ② 调试 / 日志：想看「图的执行路径、每个节点各自改了哪几个字段」  │
    │    —— 差量最直观，一眼看出谁动了谁。                           │
    │ ③ state 很大时（长 messages 列表、大文档）：                   │
    │    —— values 每步都把整个大 state 复制吐出来，量大又重复；      │
    │       updates 只吐变动字段，省带宽、省内存、省眼睛。            │
    │ ④ 多节点并发（同一步多个节点跑）：                             │
    │    —— updates 会把【每个】节点的改动分别吐出来，看得清是谁改的； │
    │       values 只给合并后的一份快照，分不清来源。                 │
    └────────────────────────────────────────────────────────────┘

    ┌─ 选 values 的场景（要「全貌」时才用）──────────────────────┐
    │ ① 前端要实时渲染「完整状态面板」：每步都把整个 state 画出来。   │
    │ ② 下游逻辑需要【组合多个字段】才能判断：                       │
    │    —— 比如要同时看 question + context + answer 三者当前值，    │
    │       用 updates 还得自己攒，用 values 一步到位。              │
    │ ③ 做「时间旅行 / 回放」：想保存每一帧完整快照，事后逐帧回看。   │
    │ ④ 字段之间有依赖、想确认「某节点跑完后整体是否自洽」时。        │
    └────────────────────────────────────────────────────────────┘

    ── 30 秒决策树 ──
       要「逐 token 打字机」？           → 都不是，用 messages
       要「节点中途推进度」？           → 都不是，用 custom
       只想知道「谁动了、动了啥」？      → updates ✅（默认首选）
       必须拿到「当前完整 state 全貌」？ → values ✅
       state 很大 / 想省流量？          → updates ✅
       只想要最终结果？                → 直接 graph.invoke() 最省事

    ⚠️ 常见误区：别用 values 去「拼凑最终结果」——那直接 invoke() 即可。
       values 的价值永远在「过程中的每一帧」，不在最后那一帧。
    ══════════════════════════════════════════════════════════════
    """
    print("\n=== 03. values 完整状态快照 ===")
    graph = _build_qa_graph()
